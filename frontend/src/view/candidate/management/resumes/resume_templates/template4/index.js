...
function Skill({ infor, index, bgColor }) {
  const { cvSkills } = useContext(TemplateContext);
...
function Certificate({ infor, index, bgColor }) {
  const { cvCertificates } = useContext(TemplateContext);
  const { cvMode } = useContext(CandidateContext);
  const [name, setName] = useState("");
...
function Prize({ infor, index, bgColor }) {
  const { cvPrizes } = useContext(TemplateContext);
  const { cvMode } = useContext(CandidateContext);
  const [name, setName] = useState("");
  const [receiveDate, setReceiveDate] = useState("");
  useEffect(() => {
...
function Education({ infor, index, menu, bgColor }) {
  const { cvEducations } = useContext(TemplateContext);
  const { cvMode } = useContext(CandidateContext);
...
function Experience({ infor, index, bgColor }) {
  const { cvExperiences } = useContext(TemplateContext);
  const { cvMode } = useContext(CandidateContext);
  const [name, setName] = useState("");
...
function Project({ infor, index, menu, bgColor }) {
  const { cvProjects } = useContext(TemplateContext);
  const { cvMode } = useContext(CandidateContext);
  const [name, setName] = useState("");
...
function Activity({ infor, index, menu, bgColor }) {
  const { cvActivities } = useContext(TemplateContext);
  const { cvMode } = useContext(CandidateContext);
...
function Other({ infor, index, bgColor }) {
  const { cvOthers } = useContext(TemplateContext);
  const { cvMode } = useContext(CandidateContext);
  const [name, setName] = useState("");
  const [description, setDescription] = useState("");
  useEffect(() => {
...
function SkillPart({ bgColor }) {
  const { basicInfor, cvSkills, setCvSkills, register } = useContext(TemplateContext);
  const { cvMode } = useContext(CandidateContext);
  return (
...
function CertificatePart({ bgColor }) {
  const { basicInfor, cvCertificates, setCvCertificates, register } = useContext(TemplateContext);
  const { cvMode } = useContext(CandidateContext);
  return (
    <InforPart
      disabled={cvMode === "READ"}
...
function PrizePart({ bgColor }) {
  const { basicInfor, cvPrizes, setCvPrizes, register } = useContext(TemplateContext);
  const { cvMode } = useContext(CandidateContext);
  return (
...
function EducationPart({ bgColor }) {
  const { basicInfor, cvEducations, setCvEducations, register } = useContext(TemplateContext);
  const { cvMode } = useContext(CandidateContext);
  return (
...
function ExperiencePart({ bgColor }) {
  const { basicInfor, cvExperiences, setCvExperiences, register } = useContext(TemplateContext);
  const { cvMode } = useContext(CandidateContext);
  return (
    <InforPart
      disabled={cvMode === "READ"}
...
function ProjectPart({ bgColor }) {
  const { basicInfor, cvProjects, setCvProjects, register } = useContext(TemplateContext);
  const { cvMode } = useContext(CandidateContext);
  return (
...
function ActivityPart({ bgColor }) {
  const { basicInfor, cvActivities, setCvActivities, register } = useContext(TemplateContext);
  const { cvMode } = useContext(CandidateContext);
  return (
